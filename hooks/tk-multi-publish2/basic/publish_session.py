...
class CinemaSessionPublishPlugin(HookBaseClass):
    """
    Plugin for publishing an open cinema session.
    ...
    def settings(self):
        ...
        base_settings = super(CinemaSessionPublishPlugin, self).settings or {}

        # settings specific to this class
        cinema_publish_settings = {
            "Publish Template": {
                "type": "template",
                "default": None,
        ...
        base_settings.update(cinema_publish_settings)
    ...
    def accept(self, settings, item):
        ...
        if not path:
            ...
            self.logger.warn(
                "The Cinema session has not been saved.",
                extra=_get_save_as_action(),
            )

        self.logger.info(
            "Cinema '%s' plugin accepted the current Cinema session."
            % (self.name,)
        )
    ...
    def validate(self, settings, item):
            ...
            if not work_template.validate(path):
                self.logger.warning(
                    ...
                    extra={
                        "action_button": {
                            "label": "Save File",
                            "tooltip": "Save the current Cinema session to a "
                            "different file name",
        ...
        return super(CinemaSessionPublishPlugin, self).validate(
            settings, item
        )
    ...
    def finalize(self, settings, item):
        ...
        super(CinemaSessionPublishPlugin, self).finalize(settings, item)
