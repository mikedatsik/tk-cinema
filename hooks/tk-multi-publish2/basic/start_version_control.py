...
class CinemaStartVersionControlPlugin(HookBaseClass):
    """
    Simple plugin to insert a version number into the cinema file path if one
    does not exist.
    """
    ...
    def accept(self, settings, item):
        ...
        if path:
            ...
            if version_number is not None:
                self.logger.info(
                    "Cinema '%s' plugin rejected the current session..."
                    % (self.name,)
                )
                self.logger.info(
                    "  There is already a version number in the file..."
                )
                self.logger.info("  Cinema file path: %s" % (path,))
        ...
        self.logger.info(
            "Cinema '%s' plugin accepted the current session." % (self.name,),
    ...
    def publish(self, settings, item):
        ...
        self.logger.info(
            "A version number has been added to the Cinema file..."
        )
        self.logger.info("  Cinema file path: %s" % (version_path,))
